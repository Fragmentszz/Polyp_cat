...
if echo "$current_hostname" | grep -q "$expected_hostname"; then
    python my_train.py --exp_dir=/applications/graduate_design/model/finetuned/cat_sam --model_config=/applications/graduate_design/cat-sam/cat_sam/config/model/7_6.yaml --dataset_config=/applications/graduate_design/cat-sam/cat_sam/config/train_dataset.yaml
else
