...
class Reins_Attention3(nn.Module):
    ...
    def create_model(self):
        ...
        #     torch.empty([self.num_layers, self.token_length, self.embed_dims])
        # )
        self.A = nn.Parameter(torch.empty([1, self.token_length, self.embed_dims*self.embed_dims_ratio]))
        self.B = nn.Parameter(torch.empty([self.num_layers,self.embed_dims*self.embed_dims_ratio,round(self.token_dim*self.mlp_ratio)]))
