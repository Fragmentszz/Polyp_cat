...
def build_model(worker_args):
    ...
    assert worker_args.cat_type in ['cat-a', 'cat-t','reins']
    ...
    print(worker_args.ckpt_path)
    logging.info(f"now_model:{worker_args.ckpt_path}")
...
if __name__ == '__main__':
    ...
    root_dir = '/applications/graduate_design/model/finetuned/cat_sam'
    # test_list = ['LoRAReins__rein_vit_l_reins_16shot','4_layer','all_layer','EVP+4layer','EVP+all_layer','EVP_feature+4layer']
    test_list = ['LoRAReins__rein_vit_l_reins_16shot']
    model_type = ['reins',]
    reins_type = ['LoRAReins',]
    if test_args.dataset == 'divide':
        ...
        for dataset in test_datasets:
            ...
            for i,dir in enumerate(test_list):
                model_dir = os.path.join(root_dir,dir)
                model_path = os.path.join(model_dir,'best_model.pth')
                test_args.cat_type = model_type[i]
                test_args.rein_type = reins_type[i]
                model = MODEL(model_path,test_args)
